...
/*   Updated: 2022/06/30 15:54:48 by arohmann         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (split[0][0] == 'p' && split[0][1] == 'l' && split[0][2] == '\0')
		return (get_plane(scene, split, line_cnt));
	if (split[0][0] == 's' && split[0][1] == 'p' && split[0][2] == '\0')
		return (get_sphere(scene, split, line_cnt));
	if (split[0][0] == 'b' && split[0][1] == 'o' && split[0][2] == '\0')
		return (get_bowl(scene, split, line_cnt));
	if (split[0][0] == 'c' && split[0][1] == 'y' && split[0][2] == '\0')
	{
		if (get_cylinder(scene, split, line_cnt) == ERROR)
			return (error_free(ERROR, split));
		return (0);
	}
	if (split[0][0] == 't' && split[0][1] == 'b' && split[0][2] == '\0')
	{
		if (get_tube(scene, split, line_cnt) == ERROR)
			return (error_free(ERROR, split));
		return (0);
	}
	if (split[0][0] == 'c' && split[0][1] == 'l' && split[0][2] == '\0')
